...
export class EditComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.searchSubscription = this.search$.subscribe((data) => {
      if (data) {
        ...
        this.store.dispatch(new BusquedasActions.BuscarEquipos());
      }
    });
    this.route.queryParamMap.subscribe((params) => {
      ...
      if (master) {
        if (this.idMaster !== parseInt(master)) {
          ...
          this.store.dispatch([
            new MatcheosActions.LoadMasterEquipo(this.idMaster),
          ]);
        }
      } else {
  ...
  deleteMaster(idMaster: number) {
    ...
    const subscription: Subscription = ref.instance.decision.subscribe(
      (value) => {
        if (value) {
          this.store
            .dispatch(new MatcheosActions.DeleteMasterEquipo(idMaster))
            .subscribe(() => {
              this.restore();
            });
        }
  ...
  unMatch(idMatch: number) {
    this.store
      .dispatch(new MatcheosActions.UnMatchEquipo(idMatch))
      .subscribe(() => {
        this.restore();
      });
  }
  addMatch(idMatch: number) {
    this.store
      .dispatch(new MatcheosActions.MatchEquipo(idMatch, this.idMaster))
      .subscribe(() => {
        this.restore();
      });
  }
