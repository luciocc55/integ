...
export class ViewComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscription = merge(this.noResults$, this.search$).subscribe(
      (data) => {
        if (data) {
          this.store.dispatch(new MatcheosActions.LoadPacientes());
        }
      }
    );
  }
  ...
  deteleteMaster(idMaster: number) {
    this.store.dispatch(new MatcheosActions.DeleteMasterPacientes(idMaster));
  }
  unMatch(idMatch: number) {
    this.store.dispatch(new MatcheosActions.UnMatchPaciente(idMatch));
  }
}
